...
void ExplicitTNNP06Vent::explicit_timestep(CellState &Cellstate, Vector<double> &new_state){
	...
	Ek = Tent_RTONF * (log((Tent_Ko / new_state[2])));
	Ena = Tent_RTONF * (log((Tent_Nao / new_state[1])));
	Eks = Tent_RTONF * (log((Tent_Ko + Tent_pKNa * Tent_Nao) / (new_state[2] + Tent_pKNa * new_state[1])));
	Eca = 0.5 * Tent_RTONF * (log((Tent_Cao / new_state[0])));
	...
	J_INF = H_INF;
	new_state[5] = M_INF - (M_INF - new_state[5]) * exp(-dt / TAU_M);
	new_state[6] = H_INF - (H_INF - new_state[6]) * exp(-dt / TAU_H);
	new_state[7] = J_INF - (J_INF - new_state[7]) * exp(-dt / TAU_J);
	INa = GNa * new_state[5] * new_state[5] * new_state[5] * new_state[6] * new_state[7] * (Vm - Ena);
	...
	Cf2 = 80 / (1. + exp((Vm + 30) / 10));
	TAU_F2 = Af2 + Bf2 + Cf2;
	FCaSS_INF = 0.6 / (1 + (new_state[4] / 0.05) * (new_state[4] / 0.05)) + 0.4;
	TAU_FCaSS = 80. / (1 + (new_state[4] / 0.05) * (new_state[4] / 0.05)) + 2.;
	new_state[8] = D_INF - (D_INF - new_state[8]) * exp(-dt / TAU_D);
	new_state[9] = F_INF - (F_INF - new_state[9]) * exp(-dt / TAU_F);
	new_state[10] = F2_INF - (F2_INF - new_state[10]) * exp(-dt / TAU_F2);
	new_state[11] = FCaSS_INF - (FCaSS_INF - new_state[11]) * exp(-dt / TAU_FCaSS);
	ICaL = Tent_GCaL * new_state[8] * new_state[9] * new_state[10] * new_state[11] * 4 * (Vm - 15) * (Tent_F * Tent_F / (Tent_R * Tent_T)) * (0.25 * exp(2 * (Vm - 15) * Tent_F / (Tent_R * Tent_T)) * new_state[4] - Tent_Cao) / (exp(2 * (Vm - 15) * Tent_F / (Tent_R * Tent_T)) - 1.);
	...
	new_state[13] = S_INF - (S_INF - new_state[13]) * exp(-dt / TAU_S);
	new_state[12] = R_INF - (R_INF - new_state[12]) * exp(-dt / TAU_R);

	Ito = Gto * new_state[12] * new_state[13] * (Vm - Ek);
	...
	bxr2 = 1.12 / (1. + exp((Vm - 60.) / 20.));
	TAU_Xr2 = axr2 * bxr2;
	new_state[14] = Xr1_INF - (Xr1_INF - new_state[14]) * exp(-dt / TAU_Xr1);
	new_state[15] = Xr2_INF - (Xr2_INF - new_state[15]) * exp(-dt / TAU_Xr2);


	IKr = Gkr * sqrt(Tent_Ko / 5.4) * new_state[14] * new_state[15] * (Vm - Ek);
	...
	Bxs = (1. / (1. + exp((Vm - 35.) / 15.)));
	TAU_Xs = Axs * Bxs + 80;
	new_state[16] = Xs_INF - (Xs_INF - new_state[16]) * exp(-dt / TAU_Xs);

	IKs = Gks * new_state[16] * new_state[16] * (Vm - Eks);
	...
	if (cellTissue == 108 || cellTissue == 107 || cellTissue == 106) {
		...
		By = exp(3.6 + (0.11 * Vm));
		tau_y = 4000. / (Ay + By);
		new_state[17] = y_inf - (y_inf - new_state[17]) * exp(-dt / tau_y);
		ifna = Gfna * new_state[17] * (Vm - Ena);
		ifk = Gfk * new_state[17] * (Vm - Ek);
		If = ifk + ifna;
	} else {
		ifna = 0.0;
		ifk = 0.0;
	...
	INaCa = Tent_knaca * (1. / (Tent_KmNai * Tent_KmNai * Tent_KmNai + Tent_Nao * Tent_Nao * Tent_Nao)) * (1. / (Tent_KmCa + Tent_Cao)) * (1. / (1 + Tent_ksat * exp((Tent_n - 1) * Vm * Tent_F / (Tent_R * Tent_T)))) * (exp(Tent_n * Vm * Tent_F / (Tent_R * Tent_T)) * new_state[1] * new_state[1] * new_state[1] * Tent_Cao - exp((Tent_n - 1) * Vm * Tent_F / (Tent_R * Tent_T)) * Tent_Nao * Tent_Nao * Tent_Nao * new_state[0] * 2.5);

	rec_iNaK = (1. / (1. + 0.1245 * exp(-0.1 * Vm * Tent_F / (Tent_R * Tent_T)) + 0.0353 * exp(-Vm * Tent_F / (Tent_R * Tent_T))));
	INaK = Tent_knak * (Tent_Ko / (Tent_Ko + Tent_KmK)) * (new_state[1] / (new_state[1] + Tent_KmNa)) * rec_iNaK;

	IpCa = Tent_GpCa * new_state[0] / (Tent_KpCa + new_state[0]);
	...
	kCaSR = Tent_maxsr - ((Tent_maxsr - Tent_minsr) / (1 + (Tent_EC / new_state[3]) * (Tent_EC / new_state[3])));
	k1 = Tent_k1_ / kCaSR;
	k2 = Tent_k2_ * kCaSR;
	dRR = Tent_k4 * (1 - new_state[18]) - k2 * new_state[4] * new_state[18];
	new_state[18] += dt * dRR;
	new_state[19] = k1 * new_state[4] * new_state[4] * new_state[18] / (Tent_k3 + k1 * new_state[4] * new_state[4]);

	Irel = Tent_Vrel * new_state[19] * (new_state[3] - new_state[4]);
	Ileak = Tent_Vleak * (new_state[3] - new_state[0]);
	Iup = Tent_Vmaxup / (1. + ((Tent_Kup * Tent_Kup) / (new_state[0] * new_state[0])));
	Ixfer = Tent_Vxfer * (new_state[4] - new_state[0]);

	CaCSQN = Tent_Bufsr * new_state[3] / (new_state[3] + Tent_Kbufsr);
	dCaSR = dt * (Iup - Irel - Ileak);
	bjsr = Tent_Bufsr - CaCSQN - dCaSR - new_state[3] + Tent_Kbufsr;
	cjsr = Tent_Kbufsr * (CaCSQN + dCaSR + new_state[3]);
	new_state[3] = (sqrt(bjsr * bjsr + 4 * cjsr) - bjsr) / 2;

	CaSSBuf = Tent_Bufss * new_state[4] / (new_state[4] + Tent_Kbufss);
	dCaSS = dt * (-Ixfer * (Tent_Vc / Tent_Vss) + Irel * (Tent_Vsr / Tent_Vss) + (-ICaL * Tent_inversevssF2 * Tent_CAPACITANCE));
	bcss = Tent_Bufss - CaSSBuf - dCaSS - new_state[4] + Tent_Kbufss;
	ccss = Tent_Kbufss * (CaSSBuf + dCaSS + new_state[4]);
	new_state[4] = (sqrt(bcss * bcss + 4 * ccss) - bcss) / 2;

	CaBuf = Tent_Bufc * new_state[0] / (new_state[0] + Tent_Kbufc);
	dCai = dt * ((-(IbCa + IpCa - 2 * INaCa) * Tent_inverseVcF2 * Tent_CAPACITANCE) - (Iup - Ileak) * (Tent_Vsr / Tent_Vc) + Ixfer);
	bc = Tent_Bufc - CaBuf - dCai - new_state[0] + Tent_Kbufc;
	cc = Tent_Kbufc * (CaBuf + dCai + new_state[0]);
	new_state[0] = (sqrt(bc * bc + 4 * cc) - bc) / 2;
