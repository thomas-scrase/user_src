#include "TomekORudy_Anti_KCNQ1.h"

namespace oomph{

TomekORudyVentAntiKCNQ1::TomekORudyVentAntiKCNQ1() : CellModelBase()
{
	Intrinsic_dt = 0.02;
...
bool TomekORudyVentAntiKCNQ1::compatible_cell_types(const unsigned& cell_type)
{
	switch(cell_type){
...
inline void TomekORudyVentAntiKCNQ1::return_initial_membrane_potential(double &v, const unsigned &cell_type)
{
	// v = -88.7638;
	switch(cell_type){
		case 100:
...
inline bool TomekORudyVentAntiKCNQ1::return_initial_state_variable(const unsigned&n, double& v, const unsigned& cell_type)
{	
	double STATES[3][59] = 
	{
...
inline void TomekORudyVentAntiKCNQ1::extract_black_box_parameters_TomekORudyVent(CellState &Cellstate)
{

}

void TomekORudyVentAntiKCNQ1::explicit_timestep(CellState &Cellstate, Vector<double> &new_state)
{
	...
	const double Anti_KCNQ1_Concentration = Cellstate.get_black_box_nodal_parameters(0);

	const double PiB_alpha=0.0315401;
	const double PiB_beta=0.620149;
	const double PiB_gamma=0.870049;
	const double PiB_delta=0.0242216;
	const double PiB_theta=4.87916;
	const double PiB_eta=0.381437;
	const double PiB_psi=0.507178;
	const double PiB_omega=0.046455;

	//Shift in activation potential
	const double VShift_alpha=-7489.44;
	const double VShift_beta=-187.393;
	const double VShift_gamma= 5.40211;
	const double VShift_delta=6027.49;
	const double VShift_eta=-51.1174;
	const double VShift_psi=399.564;
	const double VShift_omega=3929.14;


	const double A_alpha = 3.72e-003;
	const double A_beta = 2.35e-004;
	const double A_gamma = 7.25e-003;
	const double A_delta = 1.53e-003;
	const double A_theta = 1.96e-003;
	const double A_eta = 1.67e-002;
	const double A_psi = 0.00852459080491;
	const double A_omega = 0.001631335712234;

	const double B_alpha = 2.10e-001;
	const double B_beta = -2.42e-001;
	const double B_gamma = 2.43e+000;
	const double B_delta = -6.26e-001;
	const double B_eta = -1.34e+000;
	const double B_psi = 0.0124;
	const double B_omega = -0.457531296450783;

	const double Anti_KCNQ1_Compatibility_Constant = ((1.62-1.0)/(1.62-1.17))/((1.62-1.0)/(1.62-1.17) - 1.0);

	const double alpha_Iks_Base=A_alpha*exp(v*FNORT*B_alpha);
	const double beta_Iks_Base=A_beta*exp(v*FNORT*B_beta);
	const double gamma_Iks_Base=A_gamma*exp(v*FNORT*B_gamma);
	const double delta_Iks_Base=A_delta*exp(v*FNORT*B_delta);
	const double theta_Iks_Base=A_theta;
	const double eta_Iks_Base=A_eta*exp(v*FNORT*B_eta);
	const double psi_Iks_Base=A_psi*exp(v*FNORT*B_psi);
	const double omega_Iks_Base=A_omega*exp(v*FNORT*B_omega);

	const double Anti_KCNQ1_Binding_Ratio = 0.0 + (std::abs(Anti_KCNQ1_Concentration)>1e-9)*(1.0/(1.0 + pow(30.0/Anti_KCNQ1_Concentration, 1.84785961)*(pow(2.0,1.84785961)*(Anti_KCNQ1_Compatibility_Constant-1.0)/(pow(2.0,1.84785961)-Anti_KCNQ1_Compatibility_Constant))));

	const double alpha_Iks = ((1.0-Anti_KCNQ1_Binding_Ratio)*alpha_Iks_Base
		+ Anti_KCNQ1_Binding_Ratio*pow(alpha_Iks_Base,PiB_alpha)*exp(2.10e-001*PiB_alpha*VShift_alpha*FNORT));

	const double beta_Iks = ((1.0-Anti_KCNQ1_Binding_Ratio)*beta_Iks_Base
		+ Anti_KCNQ1_Binding_Ratio*pow(beta_Iks_Base,PiB_beta)*exp(-2.42e-001*PiB_beta*VShift_beta*FNORT));

	const double gamma_Iks = ((1.0-Anti_KCNQ1_Binding_Ratio)*gamma_Iks_Base
		+ Anti_KCNQ1_Binding_Ratio*pow(gamma_Iks_Base,PiB_gamma)*exp(2.43e+000*PiB_gamma*VShift_gamma*FNORT));

	const double delta_Iks = ((1.0-Anti_KCNQ1_Binding_Ratio)*delta_Iks_Base
		+ Anti_KCNQ1_Binding_Ratio*pow(delta_Iks_Base,PiB_delta)*exp(-6.26e-001*PiB_delta*VShift_delta*FNORT));

	const double theta_Iks = ((1.0-Anti_KCNQ1_Binding_Ratio)*theta_Iks_Base
		+ Anti_KCNQ1_Binding_Ratio*pow(theta_Iks_Base,PiB_theta));

	const double eta_Iks = ((1.0-Anti_KCNQ1_Binding_Ratio)*eta_Iks_Base
		+ Anti_KCNQ1_Binding_Ratio*pow(eta_Iks_Base,PiB_eta)*exp(-1.34e+000*PiB_eta*VShift_eta*FNORT));

	const double psi_Iks = ((1.0-Anti_KCNQ1_Binding_Ratio)*psi_Iks_Base
		+ Anti_KCNQ1_Binding_Ratio*pow(psi_Iks_Base,PiB_psi)*exp(0.0124*PiB_psi*VShift_psi*FNORT));

	const double omega_Iks = ((1.0-Anti_KCNQ1_Binding_Ratio)*omega_Iks_Base
		+ Anti_KCNQ1_Binding_Ratio*pow(omega_Iks_Base,PiB_omega)*exp(-0.457531296450783*PiB_omega*VShift_omega*FNORT));
