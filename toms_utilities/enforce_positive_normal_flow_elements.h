...
//LIC// 
//LIC//    Version 1.0; svn revision $LastChangedRevision: 1275 $
//LIC//
//LIC// $LastChangedDate: 2016-12-30 06:46:51 +0000 (Fri, 30 Dec 2016) $
//LIC// 
...
//LIC// 
//LIC//====================================================================
#ifndef OOMPH_IMPOSE_POSITIVE_ELEMENTS_HEADER
#define OOMPH_IMPOSE_POSITIVE_ELEMENTS_HEADER
...
{
//========================================================================
/// \short  ImposePositiveFlowElement 
...
//========================================================================
 template <class ELEMENT>
  class ImposePositiveFlowElement :
  ...
  public virtual FaceElement
  {
   ...
   /// ImposePositiveFlowElement is supposed
   /// to be attached to, and the face element ID
   ImposePositiveFlowElement
   ...
   FaceGeometry<ELEMENT>(), FaceElement()
    {
     ...
     // we need 2 additional values for each FaceElement node
     ...
     // //set them to 1 to stop singularity
     // for(unsigned i=0; i<nnode(); i++){
     //  //Get a pointer to that node
     //  BoundaryNodeBase *bnode_pt = dynamic_cast<BoundaryNodeBase*>(node_pt(i));
     //  //Get the indexes at which the lagrange multiplier and slack variables are stored
     //  unsigned bfirst_index = bnode_pt->index_of_first_value_assigned_by_face_element(Id);
     //  node_pt(i)->set_value(bfirst_index,1.0);
     //  node_pt(i)->set_value(bfirst_index+1,2.0);
     // }
    ...
    for(unsigned ipt=0;ipt<n_intpt;ipt++)
     {
      ...
      for(unsigned j=0;j<n_node;j++)
       {
        ...
        //Get the value of the lagrange multiplier
        lambda[0]+=nod_pt->value(first_index)*psi(j);
        //Get the value of the slack variable
        lambda[1]+=nod_pt->value(first_index + 1)*psi(j);
       }
      ...
      for(unsigned j=0;j<n_node;j++)
      {
        ...
        unsigned first_index=
         ...
         //!!!!!
         //!!!!!This is currently not optimal
         //!!!!!

        // Local eqn number of lamdba in the j-th node
        local_eqn=nodal_local_eqn(j,first_index);

        if (local_eqn>=0){
          //R_lambda += v_k n_k - s^2
          double temp = 0.0;
          for(unsigned k=0; k<dim_el; k++){
            temp += interpolated_u[k]*norm_vec[k];
          }
          residuals[local_eqn] += (temp - lambda[1]*lambda[1])*psi(j)*W;
          // std::cout << "l " << residuals[local_eqn] << std::endl;

          //Fill in the jacobian for lambda?
          if (flag==1){
            //Loop over the other nodes in the element
            for(unsigned jj=0; jj<n_node;jj++){
              //Get a pointer to that node
              BoundaryNodeBase *bnode_pt = dynamic_cast<BoundaryNodeBase*>(node_pt(jj));
              //Get the indexes at which the lagrange multiplier and slack variables are stored
              unsigned bfirst_index = bnode_pt->index_of_first_value_assigned_by_face_element(Id);
              //get local unknown of the slack variable associated with this node
              local_unknown=nodal_local_eqn(jj, bfirst_index+1);
              if(local_unknown>=0){
                jacobian(local_eqn, local_unknown) += -2.0*lambda[1]*psi(j)*psi(jj)*W;
              }

              //Loop over the velocity components
              for(unsigned i=0; i<dim_el+1; i++){
                //Get local unknown of the i-th velocity component in the jj-th node
                local_unknown=nodal_local_eqn(jj, u_index[i]);
                if(local_unknown>=0){
                  jacobian(local_eqn, local_unknown) += norm_vec[i]*psi(j)*psi(jj)*W;
                }
              }
            }
          }
        ...
        // Local eqn number of the slack variable in the j-th node
        local_eqn=nodal_local_eqn(j,first_index+1);
        if (local_eqn>=0){
          //R_s = -2*lambda*s
          residuals[local_eqn] += -2.0*lambda[0]*lambda[1]*psi(j)*W;
          // std::cout << "s " << residuals[local_eqn] << std::endl;

          //Fill in the jacobian for slack variable?
          if (flag==1){
            //Loop over the other nodes in the element
            for(unsigned jj=0; jj<n_node;jj++){
              //Get a pointer to that node
              ...
              //Get the indexes at which the lagrange multiplier and slack variables are stored
              unsigned bfirst_index = bnode_pt->index_of_first_value_assigned_by_face_element(Id);
              //Get local unknown of the lagrange multiplier
              local_unknown=nodal_local_eqn(jj, bfirst_index);
              if(local_unknown>=0){
                jacobian(local_eqn, local_unknown) += -2.0*lambda[1]*psi(j)*psi(jj)*W;
              }
              //Get local unknown of the slack variable
              local_unknown=nodal_local_eqn(jj, bfirst_index+1);
              if(local_unknown>=0){
                jacobian(local_eqn, local_unknown) += -2.0*lambda[0]*psi(j)*psi(jj)*W;
              }
            }
          }
        }
        ...
        for(unsigned i=0; i<dim_el+1; i++){
          // Local eqn number of the i-th velocity dof in the j-th node
          local_eqn=nodal_local_eqn(j,u_index[i]);
          if (local_eqn>=0){
            residuals[local_eqn] += lambda[0]*norm_vec[i]*psi(j)*W;
            // std::cout << "v" << i << " " << residuals[local_eqn] << std::endl;

            //Fill in jacobian for velocity dofs?
            if (flag==1){
              //Loop over the other nodes in the element
              for(unsigned jj=0; jj<n_node;jj++){
                //Get a pointer to that node
   ...
   /// divided into: The ImposePositiveFlowElement is responsible for 1(2) of
   ...
   unsigned ndof_types() const
   {
    ...
    // we have a lagrange multiplier and a slack variable for all values of dim
    //  in addition we have the number of constrained velocity dof types which
    //  is dim+1
    // In total we have dim+1+2 = dim+3
    return this->dim()+2;
   }
   ...
   /// 
   /// For the ImposePositiveFlowElement, we need to classify both the 
   ...
   /// associated with the ImposePositiveFlowElement. Let Lp1 and Lp2 be the 
   /// two Lagrange multiplier dof types associated with the 
   /// ImposePositiveFlowElement. Then we have:
   /// 
   /// 0  1  2  3  4
   /// up vp wp L1 L2
  ...
  }; // End of ImposePositiveFlowElement class
