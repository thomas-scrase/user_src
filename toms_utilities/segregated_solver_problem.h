 ...
 class SegregatableCellProblem : public virtual Problem
  {
   ...
   SegregatableCellProblem() 
    {
     ...
     Vector<Mesh*> Group_1_mesh_pts;
     Vector<Mesh*> Group_2_mesh_pts;
   ...
   //The total number of variables
   unsigned n_values_group_2 = 0;
  
   // Back up the pointers to the submeshes in the original problem
   // so we can restore the problem when we're done
   unsigned orig_n_sub_mesh_group_2=nsub_mesh();

   Value_is_pinned_group_2.resize(orig_n_sub_mesh_group_2);

   Orig_sub_mesh_pt_group_2.resize(orig_n_sub_mesh_group_2);

   for (unsigned i=0;i<orig_n_sub_mesh_group_2;i++)
   {
     Orig_sub_mesh_pt_group_2[i]=mesh_pt(i);

     unsigned n_elem = Orig_sub_mesh_pt_group_2[i]->nelement();

     Value_is_pinned_group_2[i].resize(n_elem);

     for(unsigned e=0; e<n_elem; e++){
       ELEMENT_TYPE* elem_pt = dynamic_cast<ELEMENT_TYPE*>(Orig_sub_mesh_pt[i]->element_pt(e));
       unsigned n_node = elem_pt->nnode();
       //Loop over nodes
       Value_is_pinned_group_2[i][e].resize(n_node);
       for(unsigned l=0; l<n_node; l++){
         Value_is_pinned_group_2[i][e][l].resize(elem_pt->required_nvalue(l));
         //Loop over cell variables and fill
         for(unsigned k=0; k<elem_pt->required_nvalue(l); k++){
           Value_is_pinned_group_2[i][e][l][k]=elem_pt->node_pt(l)->is_pinned(k);
           n_values++;
         }
       }
     }
   ...
   //Resize storage for the previous values
   Previous_value_group_2.resize(n_values_group_2);
   ...
   if (Use_pointwise_aitken)
   {
     Pointwise_aitken_value.resize(n_values_group_2);
     for (unsigned i=0;i<n_values_group_2;i++)
     {
      Pointwise_aitken_value[i].resize(3);
     }
   }
   ...
   /// convergence check
   Vector<double> Previous_value_group_1;

   Vector<double> Previous_value_group_2;
