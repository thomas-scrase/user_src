 ...
 * Update: Memory allocate and deallocate, verified with valgrind --leak-check=full...
 * Haibo Ni <haibo.ni0822@gmail.com>
 * Fri 17 Jul 2015 11:10:10 BST

 * Potential problem with this code:
 * be adviced the existance of possible problem in shallow copy... to be updated with deep copy oprator...

 *
 * This is a C++ version of the LSODA library.
 *
 * Author           : Ismail Adeniran <ismail.adeniran@gmail.com>
 * Date             : 16-04-2014
 ...
 *
 * It is a re-implementation of the C version by Heng Li <lh3lh3@gmail.com>, Heng Li <lh3@sanger.ac.uk>
 * http://lh3lh3.users.sourceforge.net/solveode.shtml
 * https://github.com/lh3/misc/blob/master/math/lsoda.c
 *
 * Compile with: g++ -std=c++11 -O3 -m64 -o lsoda lsoda.cpp
 *
 */
...
{
public:
    //typedef void    (LSODA::*_lsoda_f) (double, double *, double *, void *);
    //-------------
    // Constructors
    //-------------
    LSODA( ODE *odee, int neq ) : ode(odee), m_neq(neq)
    {
        g_nyh = 0;
        g_lenyh = 0;
        illin = init = ntrep = ixpr = 0;
        yh = NULL;
        wm = NULL;
        ewt = NULL;
        savf = NULL;
        ...
        itol                        = 2;
        rtol = new double [m_neq + 1];
        atol = new double [m_neq + 1];
        rtol[0]                     = 1.0E-10;
        atol[0]                     = 1.0E-10;

        for ( std::size_t i = 1; i <= m_neq; ++i ) {
            rtol[i]     = 10.0E-6;//1.0E-10;
            atol[i]     = 1.0E-6;
        }
    ...
    double ddot( int n, double *dx, int incx, double *dy, int incy )
    {
        ...
        /* Code for um_nequal or nonpositive increments.  */
     ...
     */
    void daxpy( int n, double da, double *dx, int incx, double *dy, int incy ) //***changed double *da to double da
    {
        int             ix, iy, i, m;
        ...
        /* Code for nom_nequal or nonpositive increments.  */
    ...
    /*
     ...
     c     dy(i)/dt = f(i) = f(i,t,y(1),y(2),...,y(m_neq)) (i = 1,...,m_neq).
     c
     ...
     c               subroutine f (m_neq, t, y, ydot)
     c               dimension y(m_neq), ydot(m_neq)
     ...
     c m_neq    = number of first order ode-s.
     c y      = array of initial values, of length m_neq.
     ...
     c             22 + m_neq * max(16, m_neq + 9).
     c          see also paragraph e below.
     c lrw    = declared length of rwork (in user-s dimension).
     c iwork  = integer work array of length at least  20 + m_neq.
     ...
     c switches methods accordingly, it must make use of the m_neq by m_neq
    ...
    /*
     ...
     hmin = rwork7
     */
    void lsoda_Old(void (ODE::*f)(double, double *, double *, void *), int m_neq, double *y, double *t, double tout, int itol, double *rtol, double *atol,
               int itask, int iopt, int jt,
               ...
               double rwork1, double rwork5, double rwork6, double rwork7, void *_data)
    {
        ...
        if (istate == 1) {
            _freevectors();
            /*int i;
            if (wm) for (i = 0; i <= g_nyh; ++i) { if (wm[i]) delete [] wm[i];}
            if (yh) for (i = 0; i <= g_lenyh; ++i) {if (yh[i]) delete [] yh[i];}*/
            g_nyh = g_lenyh = 0;
            yh = 0; wm = 0;
        ...
        /*
         ...
         First check legality of the non-optional inputs m_neq, itol, iopt,
         jt, ml, and mu.
         */
        ...
        if (istate == 1 || istate == 3) {
            ntrep = 0;
            if (m_neq <= 0) {
                fprintf(stderr, "[lsoda] m_neq = %d is less than 1\n", m_neq);
                terminate(istate);
                return;
            }
            if (istate == 3 && m_neq > n) {
                fprintf(stderr, "[lsoda] istate = 3 and m_neq increased\n");
                terminate(istate);
                return;
            }
            n = m_neq;
            ...
            if (jt > 2) {
                ...
                mu = iwork2;
                if (ml < 0 || ml >= n) {
                    fprintf(stderr, "[lsoda] ml = %d not between 1 and m_neq\n", ml);
                    terminate(istate);
                    return;
                }
                if (mu < 0 || mu >= n) {
                    fprintf(stderr, "[lsoda] mu = %d not between 1 and m_neq\n", mu);
                    terminate(istate);
                    return;
                }
            }
        ...
        if (istate == 1) {
            ...
            (ode->*f) (*t, y + 1, yh[2] + 1, _data);
            // fprintf(stderr, "hmax = %f\n", hmax);
            nfe = 1;
            /*
        ...
        if (istate == 2 || istate == 3) {
            ...
            case 3:
                ...
                successreturn(y, t, itask, ihit, tcrit);
                return;
            case 4:
                tcrit = rwork1;
                if ((tn - tcrit) * h > 0.) {
    ...
    void lsoda(void (ODE::*f)(double, double *, double *, void *), double *y, double *t, double tout, void *_data)
    {
     ...
     */
    void stoda( int m_neq, double *y, void (ODE::*f)(double, double *, double *, void *), void *_data )
    {
        ...
        while (1) {
            while (1) {
                ...
                correction(m_neq, y, f, &corflag, pnorm, &del, &delp, &told, &ncf, &rh, &m, _data);
                if (corflag == 0)
                    break;
                if (corflag == 1) {
    ...
    /*
     ...
     The quantities nq = nqcur, l = nq+1, n = m_neq, tn, and h are declared
    ...
    void scaleh( double *rh, double *pdh )
    {
         ...
         */
        *rh = std::min(*rh, rmax);
        *rh = *rh / std::max(1.0, fabs(h) * hmxi * *rh);
        /*
        ...
        if (meth == 1) {
            irflag = 0;
            *pdh = std::max(std::fabs(h) * pdlast, 0.000001);
     ...
     */
    void prja( int m_neq, double *y, void (ODE::*f)(double, double *, double *, void *), void *_data )
    {
        int             i, ier, j;
     ...
     */
    void correction(int m_neq, double *y, void (ODE::*f)(double, double *, double *, void *), int *corflag, double pnorm, double *del, double *delp, double *told,
        ...
        *del = 0.;
        ...
        while (1) {
            ...
            if (*m != 0 || meth != 1) {
                ...
                dcon = *del * std::min(1.0, 1.5 * crate) / (tesco[nq][2] * conit);
                if (dcon <= 1.) {
    ...
    //==========================
    /* int n_lsoda( double y[], int n, double *x, double xout, double eps, const double yscal[], void (ODE::*devis)(double, double *, double *, void *), void *data )
     {
         int             i, istate, itask;
         double         *_y, *atol, *rtol;
         _y =  new double [3 * (n + 1)];//  sizeof(double));
         atol = _y + n + 1;
         rtol = atol + n + 1;
         for (i = 1; i <= n; ++i) {
             _y[i] = y[i - 1];
...
private:
    ...
    double sm1[13] = { 0.0, 0.5, 0.575, 0.55, 0.45, 0.35, 0.25, 0.2, 0.15, 0.1, 0.075, 0.05, 0.025 };
    double ccmax, el0, h, hmin, hmxi, hu, rc, tn;
    int illin, init, mxstep, mxhnil, nhnil, ntrep, nslast, nyh, ierpj, iersl;
    int jcur, jstart, kflag, l, meth, miter, maxord, maxcor, msbp, mxncf, n, nq, nst, nfe, nje, nqu;
    double tsw, pdnorm;
    ...
    //    // Variables for the lsoda ODE solver
    double rwork1, rwork5, rwork6, rwork7;
    //  int    m_neq;
    // double atol[m_neq + 1], rtol[m_neq + 1], t, tout, y[m_neq + 1];
    double *atol;
    double *rtol;
    double *y;
    int    iwork1, iwork2, iwork5, iwork6, iwork7, iwork8, iwork9;
    ...
    ODE *ode;
    int m_neq;
    static int const mxstp0 = 500, mxhnl0 = 10;
...
//  }
//
//    void frhs( double t, double *y, double *ydot, void *data )
//    {
...
//            lsod.lsoda(&ODE::frhs, m_neq, y, &t, tout, itol, rtol, atol, itask, &istate, iopt, jt,
