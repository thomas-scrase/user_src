#include "refineable_monodomain_elements.h"

namespace oomph
{
	template <unsigned DIM>
	void RefineableMonodomainEquations<DIM>::
	fill_in_generic_residual_contribution_monodomain(	Vector<double> &residuals,
														...
														unsigned flag)
	{
		...
		const double peclet_st = this->pe_st();
		...
		for(unsigned ipt=0;ipt<n_intpt;ipt++){
			...
			for(unsigned l=0;l<n_node;l++){
				...
				dudt += this->du_dt_monodomain(l)*psi(l);
			...
			for(unsigned l=0;l<n_node;l++){
				...
				for(unsigned m=0;m<n_master;m++){
					...
					if(is_node_hanging){
						...
						local_eqn = this->local_hang_eqn(hang_info_pt->master_node_pt(m),u_nodal_index);
					...
					if(local_eqn >= 0){
						...
						residuals[local_eqn] -= (peclet_st*dudt + source)*test(l)*W*hang_weight;
...
template class RefineableQMonodomainElement<3,2>;
template class RefineableQMonodomainElement<3,3>;
template class RefineableQMonodomainElement<3,4>;
