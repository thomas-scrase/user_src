...
//LIC// 
//LIC//====================================================================
//Header file for AnisotropicConstitutiveLaw objects that will be used in all
   ...
   for(unsigned i=0;i<num_invariants;i++)
    {
     ...
     double energy_new = W(I,A,lambda);
...
//====================================================================
class AnisotropicMooneyRivlin : public AnisotropicStrainEnergyFunction
{

  public:
 
 /// Constructor takes the pointer to the value of the constants
 AnisotropicMooneyRivlin(double* c1_pt, double* c2_pt) : AnisotropicStrainEnergyFunction(),
 ...
 double W(const DenseMatrix<double> &gamma, const DenseMatrix<double> &A, const Vector<double> &lambda)
  {return AnisotropicStrainEnergyFunction::W(gamma, A, lambda);}

 /// Return the strain energy in terms of the strain invariants
 double W(const Vector<double> &I, const DenseMatrix<double> &A, const Vector<double> &lambda)
 {return (*C1_pt)*(I[0]-3.0) + (*C2_pt)*(I[1]-3.0);}

 
 /// \short Return the derivatives of the strain energy function with
 /// respect to the strain invariants
 void derivatives(Vector<double> &I, const DenseMatrix<double> &A, const Vector<double> &lambda, Vector<double> &dWdI)
  {
   dWdI[0] = (*C1_pt);
   dWdI[1] = (*C2_pt);
   dWdI[2] = 0.0;
...
class PoleZeroStrainEnergyFunction : public AnisotropicStrainEnergyFunction
{

public:

  PoleZeroStrainEnergyFunction(DenseMatrix<double>* a_, DenseMatrix<double>* b_, DenseMatrix<double>* k_) : AnisotropicStrainEnergyFunction(),
  a(a_),b(b_),k(k_) {}
  ...
  double W(const Vector<double> &I, const DenseMatrix<double> &A, const Vector<double> &lambda)
  {
    return AnisotropicStrainEnergyFunction::W(I,A,lambda);
  }

  void derivative(const DenseMatrix<double> &gamma, const DenseMatrix<double> &A, const Vector<double> &lambda, DenseMatrix<double> &dWdgamma)
  {
    for(int i=0;i<3;i++){
      for(int j=0;j<3;j++){
...
class AnisotropicGeneralisedMooneyRivlin : public AnisotropicStrainEnergyFunction
{
 ...
 AnisotropicStrainEnergyFunction(), Nu_pt(nu_pt), C1_pt(c1_pt), 
...
class HolzapfelOgdenStrainEnergyFunction : AnisotropicStrainEnergyFunction
{
public:
  HolzapfelOgdenStrainEnergyFunction(const Vector<double> &HOParams)  : a(HOParams[0]),
                                                                        b(HOParams[1]),
                                                                        af(HOParams[2]),
                                                                        as(HOParams[3]),
                                                                        bf(HOParams[4]),
...
private:
  ...
  // Vector<double>* Constants; //store all constants here? {a, b, af, as, bf, bs, afs, bfs}
                             //Or instead store as individual variables?
  double a;
  double b;
  double af;
  double as;
...
/// A specific AnisotropicConstitutiveLaw needs to be implement the pure 
/// virtual function
/// \code 
/// AnisotropicConstitutiveLaw::calculate_second_piola_kirchhoff_stress(...)
/// \endcode
...
///   \endcode
//==========================================================================
class AnisotropicConstitutiveLaw
 {
   protected:
  ...
  virtual void calculate_second_piola_kirchhoff_stress(
   const DenseMatrix<double> &g, const DenseMatrix<double> &G, const DenseMatrix<double> &A,
   const Vector<double> &lambda, DenseMatrix<double> &sigma)=0;
...
/// 
//=========================================================================
class AnisotropicGeneralisedHookean : public AnisotropicConstitutiveLaw
{
  ...
  AnisotropicGeneralisedHookean(double* nu_pt, double* e_pt) : 
 AnisotropicConstitutiveLaw(), Nu_pt(nu_pt), E_pt(e_pt),
  Must_delete_e(false) {}
 ...
 /// on on it.
  AnisotropicGeneralisedHookean(double* nu_pt) : 
 AnisotropicConstitutiveLaw(), Nu_pt(nu_pt), E_pt(new double(1.0)),
  Must_delete_e(true) {}
 
 
 /// Virtual destructor
 virtual ~AnisotropicGeneralisedHookean()
  {
   if (Must_delete_e) delete E_pt;
  }
...
class AnisotropicStrainEnergyFunctionConstitutiveLaw : public AnisotropicConstitutiveLaw
{
 ...
 AnisotropicStrainEnergyFunctionConstitutiveLaw(
  AnisotropicStrainEnergyFunction* const &anisotropic_strain_energy_function_pt) :
  AnisotropicConstitutiveLaw(), Anisotropic_strain_energy_function_pt(anisotropic_strain_energy_function_pt) {}
  ...
  bool requires_incompressibility_constraint()
   {
    return Anisotropic_strain_energy_function_pt->requires_incompressibility_constraint();
   }
