...
//LIC// 
//LIC//====================================================================
#ifndef OOMPH_ENTIRELY_COLLAPSIBLE_CHANNEL_MESH_TEMPLATE_CC
#define OOMPH_ENTIRELY_COLLAPSIBLE_CHANNEL_MESH_TEMPLATE_CC

//Include the headers file for collapsible channel
#include "entirely_collapsible_channel_mesh.template.h"
...
EntirelyCollapsibleChannelMesh<ELEMENT>::EntirelyCollapsibleChannelMesh(
 // const unsigned& nup, 
 ...
 : SimpleRectangularQuadMesh<ELEMENT>(
  ...
  Nx(nx), Ny(ny),
  U_Wall_pt(u_wall_pt), B_Wall_pt(b_wall_pt)
{
 // Mesh can only be built with 2D Qelements.
 MeshChecker::assert_geometric_element<QElementGeometricBase,ELEMENT>(2);

 //Create the EntirelyCollapsibleChannelMesh with the wall represented 
 //by the geometric object
 Domain_pt = new EntirelyCollapsibleChannelDomain(
                                          // nup, ncollapsible, ndown, ny, 
                                          // lup,lcollapsible, ldown, ly, 
                                          // wall_pt
                                          nx, ny,
 ...
 for (unsigned e=0;e<nelem;e++)
  {
    ...
    if (e<nx)
    {
     for (unsigned i=0;i<nnode_1d;i++)
      {
        // std::cout << "nod->bound 0" << std::endl;
       this->add_boundary_node(0,
                      this->finite_element_pt(e)->node_pt(i));
       ...
       // std::cout << "B " << zeta[0] << std::endl;
       
       // Set boundary coordinate
       this->finite_element_pt(e)->node_pt(i)->
        set_coordinates_on_boundary(0,zeta);
      }
    }
 ...
 //We have parametrised the boundary 0
 this->Boundary_coordinate_exists[0] = true;   


 //We have parametrised the boundary 2
...
//=================================================================
template<class ELEMENT>
void AlgebraicEntirelyCollapsibleChannelMesh<ELEMENT>::algebraic_node_update(
 const unsigned& t, AlgebraicNode*& node_pt)
{
 ...
 // Pointer to actual upper wall geom object (either the same as the wall object
 // or the pointer to the actual finite element)
  GeomObject* upper_geom_obj_pt=geom_object_pt[0];


 // Pointer to actual upper wall geom object (either the same as the wall object
 // or the pointer to the actual finite element)
  GeomObject* bottom_geom_obj_pt=geom_object_pt[1];
 
 // Get position vector to upper wall at previous timestep t!
 Vector<double> r_u_wall(2);
 upper_geom_obj_pt->position(t,s,r_u_wall);

 // Get position vector to bottom wall at previous timestep t!
 Vector<double> r_b_wall(2);
 bottom_geom_obj_pt->position(t,s,r_b_wall);

 // Assign new nodal coordinate
 // node_pt->x(t,0)=x_bottom+fract*(r_wall[0]-x_bottom);
 // node_pt->x(t,1)=fract*r_wall[1];

 node_pt->x(t,0)=r_b_wall[0] + (r_u_wall[0] - r_b_wall[0])*fract;
 node_pt->x(t,1)=r_b_wall[1] + (r_u_wall[1] - r_b_wall[1])*fract;
}
...
//=================================================================
template<class ELEMENT>
void AlgebraicEntirelyCollapsibleChannelMesh<ELEMENT>::setup_algebraic_node_update()
{
 ...
 for (unsigned j=0;j<nnod;j++)
  {
   ...
   //  {
     ...
     GeomObject* u_geom_obj_pt;
     Vector<double> s(1);
     // if(this->U_Wall_pt==NULL){std::cout << "U_Wall_pt null"; exit(0);}
     this->U_Wall_pt->locate_zeta(zeta,u_geom_obj_pt,s);

     // Get position vector to wall:
     Vector<double> r_u_wall(2);
     // if(u_geom_obj_pt==NULL){std::cout << "u_geom_obj_pt null"; std::exit(0);}
     u_geom_obj_pt->position(s,r_u_wall);


     GeomObject* b_geom_obj_pt;
     this->B_Wall_pt->locate_zeta(zeta,b_geom_obj_pt,s);

     // Get position vector to wall:
     Vector<double> r_b_wall(2);
     b_geom_obj_pt->position(s,r_b_wall);

     // Sanity check: Confirm that the walls are in their undeformed position
#ifdef PARANOID
     if ((std::fabs(r_u_wall[0]-x)>1.0e-14)&&(std::fabs(r_u_wall[1]-y)>1.0e-14))
      {
       std::ostringstream error_stream;
       error_stream 
        << "Upper Wall must be in its undeformed position when\n"
        << "algebraic node update information is set up!\n "
        << "x-discrepancy: " << std::fabs(r_u_wall[0]-x) << " = abs(" << r_u_wall[0] << "-" << x << ")" << std::endl
        << "y-discrepancy: " << std::fabs(r_u_wall[1]-y) << " = abs(" << r_u_wall[1] << "-" << y << ")" << std::endl;
      ...
      if ((std::fabs(r_b_wall[0]-x)>1.0e-14)&&(std::fabs(r_b_wall[1]-y)>1.0e-14))
      {
       std::ostringstream error_stream;
       error_stream 
        << "Bottom Wall must be in its undeformed position when\n"
        << "algebraic node update information is set up!\n "
        << "x-discrepancy: " << std::fabs(r_b_wall[0]-x) << std::endl
        << "y-discrepancy: " << std::fabs(r_b_wall[1]-y) << std::endl;
     ...
     geom_object_pt[0]=u_geom_obj_pt;
     geom_object_pt[1]=b_geom_obj_pt;
     ...
     ref_value[0]=(y-r_b_wall[1])/(r_u_wall[1]-r_b_wall[1]);
...
//=================================================================
template<class ELEMENT>
void RefineableAlgebraicEntirelyCollapsibleChannelMesh<ELEMENT>::update_node_update(
 AlgebraicNode*& node_pt)
{
 ...
 this->U_Wall_pt->locate_zeta(zeta_wall,geom_obj_pt,s);
