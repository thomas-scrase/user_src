...
{
	Cell_Model_Name = "TNNP06_Force_Without_ISAC";
														...
														double &Iion)
{
	...
	*/
    ...
    double ppforce_c = heav(SL - SL_c) * PCon_c * (exp(PExp_c * std::fabs(SL - SL_c)) - 1);

    double ppforce_t = sign(SL - SLrest) * PCon_t *(exp(PExp_t *std::fabs((SL - SLrest))) - 1);
    ...
    double PreloadF = sign((SLset - SLrest)) * PCon_t *(exp(PExp_t *std::fabs((SLset - SLrest))) - 1.0);
    ...
    double preload = sign(SLset - SLrest) * PCon_t *(exp(PExp_t *std::fabs(SLset - SLrest)) - 1);
    // printf("%f\n", preload);
    preload = PreloadF;
