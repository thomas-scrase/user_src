#include "TNNP06_Distinct_IKs.h"


namespace oomph{

TNNP06DistinctIKs::TNNP06DistinctIKs()
{
	Tent_R = 8314.472;
	Tent_F = 96485.3415;
	Tent_T = 310.0;
...
double TNNP06DistinctIKs::return_initial_state_variable(const unsigned &v, const unsigned &cell_type)
{
	double STATE[20];
...
double TNNP06DistinctIKs::return_initial_membrane_potential(const unsigned &cell_type)
{
	if (cell_type == 108 || cell_type == 107 || cell_type == 106){
...
void TNNP06DistinctIKs::Calculate_Derivatives(const double &Vm,
							...
							double &Iion)
{
	...
	get_iks_gating(Vm, Other_Parameters, Other_Variables, Xs_INF, Axs, Bxs, TAU_Xs); //calculate the gating variables
...
void TNNP06DistinctIKs::get_optional_output(const double &Vm,
							const Vector<double> &CellVariables,
							const double &t,
							const unsigned &cell_type,
							const double &Istim,
